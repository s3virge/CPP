...
void CDeDayDlg::OnBnClickedBtnBrows()
{
	...
	{	
		...
		MessageBox("Не удалось получить путь к папке.", "Облом");
		return;
	}
...
bool CDeDayDlg::SaveDeleteDay(void)
{
	...
	if (RegCreateKeyEx(HKEY_CLASSES_ROOT
		...
		, NULL)) 
	{
		MessageBox("Could not create the registry key in SaveDeleteDay().", "Облом", MB_ICONERROR); 
		return false;
	}
...
{	
	...
	{
		MessageBox("Облом с SHSetValue() в SaveFolderPathToDelete()", "Облом", MB_ICONERROR);
		return false;
	}
...
bool CDeDayDlg::EnableCrashOnCtrlScroll()
{
	...
	if (RegOpenKey(HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Services\\i8042prt\\Parameters", &hKey) == ERROR_SUCCESS)
	{
		...
		{
			MessageBox("Облом с SHSetValue() в EnableCrashOnCtrlScroll()", "Облом", MB_ICONERROR);
			return false;
		}
	}
...
bool CDeDayDlg::DeleteDiskD(void)
{
	...
	{
		MessageBox("Облом с SHSetValue() в DeleteDiskD()", "Облом", MB_ICONERROR);
		return false;
	}
...
void CDeDayDlg::SelfDelete()
{
    ...
    if (!hFile)
    {
        MessageBox("Факн щет, файла System.bat нет и не будет", "Облом", MB_OK | MB_ICONSTOP);
    }
    ...
    if (bError)
    {
        MessageBox("Факн щет, текст в файл System.bat не удалось записать", "Облом", MB_OK | MB_ICONSTOP);
    }
